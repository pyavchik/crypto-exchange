...
// The one visibility-aware, receiver-safe polling implementation in the
// repo, generic over its payload. Extracted mechanically from the original
// createHealthPoller (Phase 1) — every behavior below is unchanged from
// that implementation, only parameterised over the payload type T and the
// interval, which each consumer (healthPoller.ts, marketsPoller.ts) now
// supplies. See wiki/pages/findings/health-poller-illegal-invocation.md for
// why the timer-handling discipline here is load-bearing, not stylistic.
...
export interface CreatePollerOptions<T> {
  fetchImpl: (options?: { signal?: AbortSignal }) => Promise<ApiResult<T>>;
  onUpdate: (state: PollerState<T>) => void;
...
export function createPoller<T>(options: CreatePollerOptions<T>): Poller {
  const {
    fetchImpl,
    onUpdate,
    visibility,
  ...
  // instead of hiding them behind a masked "unavailable" state.
  function performFetch(): Promise<void> {
    ...
    const promise = fetchImpl({ signal: abortController.signal }).then(
      (result) => {
        controller = null;
        checking = false;
        if (stopped) return;
      ...
      (error: unknown) => {
        ...
        // Not an ApiError: a programming error (e.g. a bug in fetchImpl
