...
2026-09-16 | FINDING | 02-REVIEW.md CR-01 fixed: verifyPassword fail-open on malformed hash (Buffer.from(hex) silent truncation -> zero-length timingSafeEqual bypass); fixed in 7e2d3da (formatting: 493ba19), also fixed WR-01 (a141a75) and WR-02 (e6500bc) | findings/verify-password-fail-open, index
2026-09-16 | DECISION | Phase 3 close: keyed in-memory TTL cache + in-flight dedupe for markets/chart (not the durable upstream_checks table), last-good stale-serve fallback with one log line per failure, USD-upstream/USDT-display convention (vs_currency=usdt verified rejected live), verified Demo limits (100/min, 10k/mo) replacing the unverified marker, monthly-cap exposure accepted and documented | decisions/market-data-cache-and-stale, concepts/market-data-caching, entities/coingecko-api, index
2026-09-16 | LINT | Phase 3 transition lint (base e39073e): pages=20 orphans=0 broken_links=0 duplicates=0 unverified=3 errors=0; contradiction pass read market-data-caching, coingecko-api and order-rules against the new decision page and found and fixed one stale fact outside this phase's own touched pages (coingecko-proxy.md still cited the old unverified ~30 calls/min figure and an "open" Phase 3 TTL strategy) | decisions/coingecko-proxy, all
